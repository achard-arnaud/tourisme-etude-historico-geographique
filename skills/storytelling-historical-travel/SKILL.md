...
For advanced work there is **no maximum length** for the manuscript. Use the **content-preservation gate** and explicit dispositions instead of compression by budget. Side-story type profiles use a hard minimum, a normal target range and a **soft upper review threshold**. Crossing the soft upper threshold triggers splitting, promotion into the trunk or retyping; it never authorizes truncation of sourced matter.
