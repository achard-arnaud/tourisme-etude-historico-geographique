#!/usr/bin/env python3
"""Run27 completeness contract: every eligible claim/fragment gets a disposition.

This closes the aggregate-coverage blind spot. A claim counted once in a half-sentence
and a claim developed in a full paragraph no longer look identical: the manifest
records paragraph IDs and both gross and apportioned word counts.
"""
...
from paragraph_repair_loop import FINAL_STATUSES,validate_disposition
...
def load_promoted_fragments(project:Path)->dict[str,dict[str,Any]]:
    ...
    for path in sorted(project.glob("00_method/capture/*field_fragments.json")):
        ...
        for item in items:
            ...
            if target:
                out[str(item["id"])]=dict(item,source_path=str(path.relative_to(project)),promotes_to=target)
    return out
...
def normalize_dispositions(project:Path,run_report:dict[str,Any],claim_manifest:dict[str,dict[str,Any]])->dict[str,dict[str,Any]]:
    ...
    # Claims may be explicitly reported or inferred as included from actual manuscript markers.
    for cid in claims:
        row=raw.get(cid)
    ...
    # A promoted fragment inherits the explicit disposition of the claim it became.
    for fid,fragment in fragments.items():
        target=fragment["promotes_to"];claim_row=out.get(f"claim:{target}")
        if claim_row:
            out[f"fragment:{fid}"]={"unit_id":fid,"status":claim_row["status"],"rationale":f"Disposed through promoted claim {target}.","via_claim":target}
        elif raw.get(fid):
...
def coverage_completeness(project:Path,markdown:str,run_report:dict[str,Any])->dict[str,Any]:
    ...
    return {
        "eligible_total":len(eligible),"eligible_claims":len(claims),"eligible_promoted_fragments":len(fragments),
        "accounted_for":len(accounted),"unaccounted":unaccounted,"errors":errors,
        "status_counts":{status:sum(1 for row in dispositions.values() if row.get("status")==status) for status in sorted(FINAL_STATUSES)},
