...
SCHEMA_VERSION="1.0"; CLASS="reader_profile"; KINDS={"detour","dezoom","also","method","false_lead","portrait","object_focus","comparator","callback"}
...
def validate_reader_profile(project:Path)->tuple[list[str],list[str],int]:
    ...
    if set(priority)!=KINDS: errors.append("reader profile priority_order must enumerate every side-story kind exactly once")
