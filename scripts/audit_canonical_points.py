...
Run27 decision: until canonical_points are genuinely authored, their absence is a
warning-only migration debt. `--strict` is the future activation switch and fails if
any claim lacks a non-empty canonical-points array.
"""
...
def audit(projects=PROJECTS)->dict:
    ...
    for project in projects:
        for path,claim in load_claims(project):
            total+=1;points=claim.get("canonical_points") or claim.get("canonical_summary_points")
            row={"id":str(claim["id"]),"path":str(path.relative_to(REPO))}
    ...
    return {
        "mode":"warning_only_until_populated",
        "claims_total":total,
        "claims_with_canonical_points":len(populated),
        "claims_missing_canonical_points":len(missing),
        "coverage_percent":round((len(populated)/total*100),1) if total else 100.0,
        "missing":missing,
    }
...
def main()->int:
    ...
    print(text,end="")
    if a.strict and report["claims_missing_canonical_points"]:return 1
    return 0
