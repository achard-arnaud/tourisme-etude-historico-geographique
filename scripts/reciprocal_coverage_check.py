...
def claim_fragment_refs(claim:dict)->set[str]:
    ...
    for key in ("fragment_ids","origin_fragment_ids","input_fragment_ids"):refs|={str(x) for x in claim.get(key) or []}
    return refs
...
def density_signals(claims:list[dict],fragments:dict[str,dict],markdown:str)->list[dict]:
    ...
    for claim in claims:
        ...
        source=text_payload(claim)+" "+" ".join(text_payload(fragments[f]) for f in claim_fragment_refs(claim) if f in fragments)
        sw=len(WORD.findall(source));rw=len(WORD.findall(MARKER.sub("",block)))
        if not sw:continue
        ratio=rw/sw;warning="compression_risk" if ratio<0.45 else "stylistic_padding_risk" if ratio>3 else None
        out.append({"claim_id":cid,"source_words":sw,"rendered_words":rw,"ratio":round(ratio,3),"warning":warning})
    return out
...
def reciprocal_coverage_check(project:Path,scaffold:dict,markdown:str,instrumentation_complete:bool=False)->dict:
    ...
    counts={cid:seen.count(cid) for cid in ids};covered=sorted(cid for cid,n in counts.items() if n);absent=sorted(ids-set(covered));fragments=fragment_index(project);used=set().union(*(claim_fragment_refs(c) for c in claims)) if claims else set();density=density_signals(claims,fragments,markdown)
    return {"project":project.name,"instrumentation_complete":instrumentation_complete,"explicitly_covered_claims":covered,"unused_claims":absent if instrumentation_complete else [],"coverage_unknown_legacy":[] if instrumentation_complete else absent,"over_mentioned":sorted(cid for cid,n in counts.items() if n>2),"mention_count":{cid:n for cid,n in sorted(counts.items()) if n},"unused_fragments":sorted(set(fragments)-used),"side_story_claim_overlap":side_story_overlap(project,scaffold),"density_signals":density,"density_warnings":[x for x in density if x["warning"]],"notes":["Legacy prose without claim markers is unknown coverage, not automatically unused.","More than two direct citations is a callback-review signal.","Density warnings are advisory only and never block narrative output."]}
