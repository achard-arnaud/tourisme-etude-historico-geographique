...
OUTPUT_NAMES={
    "pre":{
        "manuscript":"Sri_Lanka_pre_1948_run27_from_scratch.md",
        "ledger":"review_ledger_run27_pre.json",
        "run_report":"run27_report_pre.json",
        "coverage":"run27_coverage_pre.json",
        "docx":"Sri_Lanka_pre_1948_run27_from_scratch.docx",
    },
    "post":{
        "manuscript":"Sri_Lanka_post_1948_run27_from_scratch.md",
        "ledger":"review_ledger_run27_post.json",
        "run_report":"run27_report_post.json",
        "coverage":"run27_coverage_post.json",
        "docx":"Sri_Lanka_post_1948_run27_from_scratch.docx",
    },
}


def claim_total(manifest:dict)->int:
    return sum(int(row.get("claims",0)) for row in manifest.get("arc_summaries") or [])


def prepare()->dict:
    corpora={}
    for key,project in PROJECTS.items():
        packet_dir=project/"09_output"/"from_scratch"/"run27_packets"
        manifest=build_packets(project,packet_dir)
        if manifest.get("contamination_check",{}).get("reader_prose_loaded") is not False:
            raise RuntimeError(f"{key}: reader prose contamination detected")
        total=claim_total(manifest)
        if total<=0:
            raise RuntimeError(f"{key}: empty claim universe")
        corpora[key]={
            "project":str(project.relative_to(REPO)),
            "packet_dir":str(packet_dir.relative_to(REPO)),
            "arc_packets":len(manifest.get("packet_paths") or []),
            "eligible_claims":total,
            "reader_prose_loaded":False,
            "outputs":OUTPUT_NAMES[key],
        }
    canonical=audit_canonical_points()
    request={
        "schema_version":"1.0",
        "run_id":"run27-final-from-scratch",
        "status":"ready_for_semantic_generation",
        "mode":"from_scratch",
        "corpora":corpora,
        "generation_contract":{
        ...
        "canonical_points":{
            ...
            "claims_total":canonical.get("claims_total"),
            "populated":canonical.get("populated"),
            "gate":"warning-only until a genuine editorial migration populates the field",
        },
        "closure_conditions":[
            "both review ledgers complete and independently Sarah-reviewed",
            "required return targets all resolved_marker or resolved_anchor after persisted research fallback",
            "coverage_completeness.unaccounted == [] for both corpora",
            "no silent evidence-status upgrade",
            "Run25-vs-Run27 residual content gap fully explained",
            "DOCX visual side-story boundary QA green",
        ],
    }
    path=REPO/"docs"/"RUN27_FINAL_RUN_REQUEST.json"
