...
def load_sources(root):
    ...
    for p in sorted((root/'05_sources').glob('source_register*.json')):
        ...
        if not isinstance(data, list):
            errors.append(f'invalid source register: {p}: expected JSON list'); continue
        for item in data:
            if not isinstance(item, dict) or not item.get('id'):
                errors.append(f'invalid source entry in {p.name}: {item!r}'); continue
            sid = item['id']
            if sid in sources:
                errors.append(f'duplicate source id across registers: {sid}'); continue
            sources[sid] = item
...
def validate_wiki(root, sources, errors):
    wiki = root/'03_wiki'
    pages = [p for p in wiki.rglob('*.md') if p.name.lower()!='readme.md'] if wiki.exists() else []
    seen = set()
    for p in pages:
...
def validate_v3_claim(c, p, sources, errors):
    ...
    if c.get('type') not in VALID_TYPES:
        errors.append(f'invalid statement type: {cid}')
    if c.get('causal_role') not in VALID_CAUSAL_ROLES:
        errors.append(f'invalid causal_role: {cid}')
    path_arc = p.parents[1].name
    if c.get('arc') != path_arc:
        errors.append(f'arc/path mismatch: {cid} ({c.get("arc")} != {path_arc})')
    if c.get('type') == 'metric':
        metric = c.get('metric')
        if not isinstance(metric, dict) or any(metric.get(k) in (None, '') for k in METRIC_FIELDS):
            errors.append(f'metric metadata incomplete: {cid}')
    ...
    if major and sids and not c.get('bounded_by'):
        ...
        if len(sids) < 2 or len(domains) < 2:
            errors.append(f'independent corroboration missing: {cid}')
...
def validate_v3_bridge(b, errors):
    ...
    for field in BRIDGE_FIELDS:
        if b.get(field) in (None, '', []):
            errors.append(f'bridge missing {field}: {bid}')
    if b.get('from_claim') == b.get('to_claim'):
        errors.append(f'bridge self-reference: {bid}')
...
def main():
    ...
    errors=[]; warnings=[]; strict = contract_version(root) >= 3
    sources,se = load_sources(root); errors += se
    for sid,source in sources.items():
