...
VALID_CONF=set('ABCDU');VALID_ZOOMS={f'Z{i}' for i in range(5)};VALID_TIERS={f'T{i}' for i in range(6)};DEBUG=bool(os.environ.get('SKILL_DEBUG'))
...
def parse_frontmatter(path):
    ...
    for line in block.splitlines():
        if ':' in line and not line.lstrip().startswith('- '):k,v=line.split(':',1);data[k.strip()]=v.strip().strip('"\'')
    return data,None
...
def validate_graph_edges(root,sources,errors):
    ...
    return count
def main():
    root=Path(sys.argv[1] if len(sys.argv)>1 else '.');errors=[];warnings=[];sources,se=load_sources(root);errors+=se
    ...
    wiki_count=validate_wiki(root,sources,errors);graph_count=validate_graph_edges(root,sources,errors);side_count=recap_count=map_count=profile_count=0;coverage={'tracked':0,'discovered':0,'untracked':0}
    ...
    if (root/'00_method/reader_profile.json').exists():e,w,profile_count=validate_reader_profile(root);errors+=e;warnings+=w
    ...
    if errors:return 1
    print(f"QA OK: {len(claims)} claims, {len(sources)} sources, {wiki_count} wiki pages, {graph_count} graph edges, {node_count} graph nodes, {side_count} side stories ({coverage['tracked']}/{coverage['discovered']} tracked, {coverage['untracked']} untracked), {recap_count} arc recaps, {map_count} map assets, {profile_count} reader profile, {len(warnings)} warnings");return 0
