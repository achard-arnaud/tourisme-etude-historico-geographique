...
def side_story_coverage(project):
    canonical=canonical_markdown_path(project).read_text(encoding="utf-8");discovered=discover_side_story_fragments(canonical);aliases=[]
    for _,item in load_side_stories(project):aliases.extend([item.get("title","")]+list((item.get("content") or {}).get("legacy_titles") or []))
    alias_norm={_strip_markup(x) for x in aliases if x};tracked=[];untracked=[]
    for frag in discovered:
        title=_strip_markup(frag["title"]);matched=any(title==a or (len(title)>12 and (title in a or a in title)) for a in alias_norm)
...
def validate_side_stories(project,*,check_render=True):
    ...
    try:
        ...
        if not stories and not (project/"00_method/output_state.json").exists():return [],[],0,{"discovered":0,"tracked":0,"untracked":0,"untracked_fragments":[]}
    ...
    if require_complete and coverage["untracked"]:errors.append(f"side-story coverage incomplete: {coverage['tracked']}/{coverage['discovered']} tracked; {coverage['untracked']} untracked: {coverage['untracked_fragments']}")
