...
def _violations(result:Any)->list[dict[str,Any]]:
    ...
    for value in getattr(result,"violations",[]) or []:
        if hasattr(value,"rule"):
            ...
            out.append({"category":str(value.get("category","")),"rule":str(value.get("rule","")),"message":str(value.get("message",value))})
        else:
            out.append({"category":"","rule":"","message":str(value)})
    return out

def repair_paragraph(unit_id:str,draft_fn:Callable[[int,list[dict[str,Any]]],str],review_fn:Callable[[str],Any],*,route_to_side_story:Callable[[str],str|None]|None=None,max_attempts:int=MAX_ATTEMPTS)->RepairDisposition:
    ...
    for attempt in range(1,max_attempts+1):
        ...
        result=review_fn(text);passed=bool(getattr(result,"passed",False));violations=_violations(result)
