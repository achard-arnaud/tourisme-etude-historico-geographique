...
class Pre1948FunctionalBaselineTests(unittest.TestCase):
    ...
    def test_claims_persist_type_and_arc(self):
        paths=list(PROJECT.glob('01_arcs/*/claims/*.json'));self.assertEqual(9,len(paths))
        for p in paths:
            c=json.loads(p.read_text(encoding='utf-8'));self.assertIn('type',c);self.assertEqual(p.parents[1].name,c['arc'])
    def test_eight_hil_baselines_and_three_arc_recaps_are_durable(self):
        self.assertEqual(8,len(list((PROJECT/'02_hil').glob('HIL-*/baseline.json'))));self.assertEqual(3,len(list((PROJECT/'09_output/arc_recaps').glob('*.json'))))
