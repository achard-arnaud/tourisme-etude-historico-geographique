...
class CompositionPipelineTests(unittest.TestCase):
    ...
    def test_arc_recap_materializer_is_deterministic_and_idempotent(self):
        ...
        source=(PRE/'09_output/report.md').read_text(encoding='utf-8');once,count=materialize_arc_recaps(PRE,source);twice,count2=materialize_arc_recaps(PRE,once);self.assertEqual(3,count);self.assertEqual(3,count2);self.assertEqual(once,twice)
        for rid in ('RECAP-A06','RECAP-A07','RECAP-A08'):self.assertEqual(1,once.count(f'[ARC-RECAP:{rid}]'))
        self.assertLess(once.index('[ARC-RECAP:RECAP-A06]'),once.index('## 4. 1744–1763'));self.assertLess(once.index('[ARC-RECAP:RECAP-A07]'),once.index('## 6. Bridge vers 1948'));self.assertLess(once.index('[ARC-RECAP:RECAP-A08]'),once.index('## 6. Bridge vers 1948'))
    ...
    def test_reader_profiles_are_deterministic_and_child_and_historian_keep_all_kinds(self):
        kinds={'detour','dezoom','also','method','false_lead','portrait','object_focus','comparator','callback','analytical_focus'}
    ...
    def test_map_asset_requires_vision_then_human_approval(self):
        sys.path.insert(0,str(ROOT/'scripts'));from map_asset_contract import validate_map_assets
        with tempfile.TemporaryDirectory() as tmp:
            p=Path(tmp);(p/'01_arcs/A1').mkdir(parents=True);(p/'09_output/map_assets').mkdir(parents=True);(p/'09_output/side_stories').mkdir(parents=True);(p/'00_method').mkdir();(p/'00_method/reader_profile.json').write_text(json.dumps({'language':'fr'}),encoding='utf-8')
            candidate={'schema_version':'1.0','class':'map_asset','id':'MAP1','status':'human_approved','story_ref':{'type':'arc','id':'A1'},'source':{'url':'https://example.test/map','image_path':'map.png','language':'fr'},'historical_context':{'map_date':'1815'},'vision_review':{'checks':{'geography_matches':True,'historical_scope_matches':True,'labels_legible':True,'no_obvious_anachronism':True}},'human_review':{'status':'pending','reviewed_at':None},'fragment':{'caption':'x','what_it_shows':'x','why_here':'x','limits':'x'},'placement':{'subsection_ref':'x'}};(p/'09_output/map_assets/MAP1.json').write_text(json.dumps(candidate),encoding='utf-8');errors,_,_=validate_map_assets(p);self.assertTrue(any('human approval' in e for e in errors))
